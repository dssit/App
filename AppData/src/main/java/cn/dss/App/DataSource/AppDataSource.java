...
//               starsid     endsid
//        String sid_start=args[0];
//        String sid_end=args[1];
        ...
        try {
            int start_sid = Integer.parseInt(args[0]);
            int end_sid = Integer.parseInt(args[1]);
...
//            String iid = "0" + id + "";
            int sid;
//             sid 学生的id
            for (sid = Integer.parseInt(args[0]); sid < Integer.parseInt(args[1]); sid += 1) {
                String name = getname();
                int Time;
//                每个人考了4次
                for (Time = 0; Time < 4; Time += 1) {
                    double min = 50;
                    double max = 100;
                    Random random = new Random();
                    double Chinese;
                    String Chinese_s, Maths_s, English_s;
                    double Maths;
                    double English;

                    //随机的生成分数

                    Chinese = random.nextDouble() * 100;
                    Maths = random.nextDouble() * 100;
                    English = random.nextDouble() * 100;
                    while (Chinese < 30 || Maths < 30 || English < 30) {
                        Chinese = random.nextDouble() * 100;
                        Maths = random.nextDouble() * 100;
                        English = random.nextDouble() * 100;
                    }

                    Chinese_s = String.format("%.2f", Chinese);
                    Maths_s = String.format("%.2f", Maths);
                    English_s = String.format("%.2f", English);
                    String datalog = "0" + id + "," + sid + "," + name + "," + Chinese_s + "," + Maths_s + "," + English_s + "," + "0" + Time;
                    System.out.println(datalog);
//
                    String logFile = args[2];
//                    String logFile = "/home/centos/calllog/calllog.log";
